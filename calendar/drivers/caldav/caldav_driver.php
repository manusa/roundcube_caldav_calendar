	...
	*/
        ...
        foreach ($form as $tab) {
            if (!empty($tab['fieldsets']) && is_array($tab['fieldsets'])) {
                ...
                foreach ($tab['fieldsets'] as $fieldset) {
                    ...
                    if ($subcontent) {
                        $content .= html::tag('fieldset', null, html::tag('legend', null, rcube_utils::rep_specialchars_output($fieldset['name'])) . $subcontent) ."\n";
                    }
                }
            }
            else {
            ...
            if ($content) {
                $this->form_html .= html::tag('fieldset', null, html::tag('legend', null, rcube_utils::rep_specialchars_output($tab['name'])) . $content) ."\n";
            }
        }
        ...
        if ( is_array( $form['content'] ) && ! empty( $form['content'] ) )
        {
            if(isset($form["style"]) && $form["style"] == "table")
            {
                ...
                foreach ($form['content'] as $col => $colprop) {
                    ...
                    $table->add('title', html::label($colprop['id'], rcube_utils::rep_specialchars_output($label)));
                ...
                foreach ( $form['content'] as $col => $colprop ) {
                    ...
                    $content .=
                        html::div("calendar-input",
                            html::label( $colprop['id'], rcube_utils::rep_specialchars_output( $label ) ) .
                            html::br() .
                            $colprop['value'] );
                }
            }
    ...
    private function _is_synced($cal_id)
    {
        ...
        $query = $this->rc->db->query(                                                                                   
            ...
            "SET caldav_last_change = CURRENT_TIMESTAMP WHERE calendar_id = ? AND ".                                                 
            $this->_unix_timestamp('caldav_last_change') ." + ? <= ".$this->_unix_timestamp($this->rc->db->now()),                   

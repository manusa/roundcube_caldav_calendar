...
class oauth_token_storage implements StorageInterface
{
    ...
    public function storeAccessToken(AccessToken $access_token) {
        $result = $this->rc->db->query(
            "INSERT INTO ".$this->db_access_token." (provider_id, client_config_id, user_id, scope, access_token, token_type, expires_in, issue_time) ".
            "VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    public function getAccessToken($client_config_id, Context $context) {
        ...
        if($result && ($data = $this->rc->db->fetch_assoc($result))) {
    ...
    public function deleteAccessToken(AccessToken $access_token) {
        ...
        return $this->rc->db->affected_rows($result) == 1;
    }

    public function storeRefreshToken(RefreshToken $refresh_token) {
        $result = $this->rc->db->query(
            "INSERT INTO ".$this->db_refresh_token." (provider_id, client_config_id, user_id, scope, refresh_token, issue_time) ".
            "VALUES(?, ?, ?, ?, ?, ?)",
    ...
    public function storeState(State $state) {
        $result = $this->rc->db->query(
            "INSERT INTO ".$this->db_states." (provider_id, client_config_id, user_id, scope, issue_time, state) ".
            "VALUES(?, ?, ?, ?, ?, ?)",
